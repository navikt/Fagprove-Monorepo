...
import { manuellBeslutningApiPath, type ManuellBeslutningRequest } from '../../../lib/foreldrepenger';
import {
...
describe('ManuellBeslutningPanel', () => {
  it('shows validation error when begrunnelse is empty', async () => {
    ...
    render(<ManuellBeslutningPanel sak={seedManuellVurderingSakResponse} onDecisionSaved={vi.fn()} />);

    await user.click(screen.getByRole('button', { name: 'Innvilg manuelt' }));

    expect(screen.getByText('Begrunnelse må fylles ut før du kan fatte manuelt vedtak.')).toBeInTheDocument();
  ...
  it.each<[string, 'INNVILGELSE' | 'AVSLAG']>([
    ...
    server.use(
      http.post(manuellBeslutningApiPath('1004'), async ({ request }) => {
        ...
        return HttpResponse.json(createManualDecisionSakResponse(received.type, received.begrunnelse, received.besluttetAv));
      }),
    );
    render(<ManuellBeslutningPanel sak={seedManuellVurderingSakResponse} onDecisionSaved={vi.fn()} />);
    ...
    await waitFor(() =>
      expect(received).toEqual({ type: expectedType, begrunnelse: 'Kontrollert.', besluttetAv: 'Kari Saksbehandler' }),
    );
  });
  ...
  it('calls onDecisionSaved with updated sak on success', async () => {
    ...
    render(<ManuellBeslutningPanel sak={seedManuellVurderingSakResponse} onDecisionSaved={onDecisionSaved} />);
  ...
  it('shows error alert when submission fails', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('Kunne ikke lagre manuell beslutning');
